...
int bfs2()
{
    ...
    while(!q.empty())
    {
        ...
        for(auto i : adj[p.first])
        {
            if(parent[i] == -1 && flow[p.first][i] > 0)
            {
                parent[i] = p.first;
                q.push({i, flow[p.first][i]});
                if(i == n)
                    return flow[p.first][i];
            }
        }
    }
    return 0;
...
int main()
{
    // Edmonds-Karp (Count & Print Augmenting Paths)
    ios::sync_with_stdio(0); cin.tie(0); cout.tie(0);
    cin >> n >> m;
    adj.clear(); adj.resize(n+1);
    flow.clear(); flow.resize(n+1, vi(n+1, 0));
    capacity.clear(); capacity.resize(n+1, vi(n+1, 0));
