...
int n, m;
vvi adj; vvl dp;
ll hamiltonian_paths(int mask, int node)
{
    if(node == n && __builtin_popcount(mask) == n)
        return 1; 
    if(node == n || __builtin_popcount(mask) == n)
        return 0;
    ...
    {
        if(!(mask & (1 << (i-1))))
            c += hamiltonian_paths(mask | (1 << (i-1)), i);
    }
    return dp[mask][node] = c % MOD;
}
int main()
{
    // Hamiltonian Paths
    ...
    dp.clear(); dp.resize(1<<n, vl(n+1, -1));
    cout << hamiltonian_paths(1, 1) << endl;
}