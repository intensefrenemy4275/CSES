...
pii solve(int i)
{
    ...
    for(int f = 0; f<n; f++)
    {
        if((1 << f) & i)
        {
            pii prev = solve(i^(1 << f));
            prev.second += (ll)w[f];
            if(prev.second > x){
                prev.first += 1;
                prev.second = w[f];
            }
            if(best.first == -1 || best.first > prev.first)
                best = prev;
            else if(best.first == prev.first && best.second > prev.second)
                best = prev;
        }
    }
    return dp[i] = best;
